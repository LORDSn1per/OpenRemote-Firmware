...
**Date:** 2026-08-17b (supersedes `OPENREMOTE_AI_HANDOFF_2026-08-17.md`)
**Reason for handoff:** Adds one researched finding (DFS/CPU-frequency limits during BLE idle) on top of
the same day's boot-speed handoff. Nothing is mid-edit; everything below is committed.
...
No firmware change since the prior handoff — 3.04 is still current. This update is research/documentation
only.

## NEW: why CPU frequency can't drop below 80MHz during BLE-connected idle

The user asked whether, during 2.95's "less aggressive" BLE connection profile, the CPU clock also drops,
and whether it could go lower than 80MHz. It cannot, and the reason is worth recording precisely because
it's a genuine architectural ceiling, not a missed setting:

- `configureApplicationPowerMode()` always returns `false` — the code comment says *"Reconfiguring global
  DFS after the live UART/BLE stack starts is unsafe on Rev 5"*, and separately, the pinned Arduino
  framework's prebuilt `sdkconfig` (checked directly on disk, `~/.platformio/packages/framework-
  arduinoespressif32-libs/esp32s3/sdkconfig`) has neither `CONFIG_PM_ENABLE` nor
  `CONFIG_FREERTOS_USE_TICKLESS_IDLE` set. Real ESP-IDF dynamic frequency scaling (which could idle the CPU
  down toward ~10MHz between BLE connection events) is genuinely unavailable on this build.
- Because DFS is unavailable, the fallback is a hardcoded manual clock drop:
  `setCpuFrequencyMhz(BLE_CONNECTED_IDLE_MAX_CPU_MHZ)`, where `BLE_CONNECTED_IDLE_MAX_CPU_MHZ = 80`. 80MHz
  is a fixed floor, not a dial — nothing in the current build can take it lower while BLE stays connected.
- **This is not a prebuilt-library limitation you can flag-flip away in `platformio.ini`.** `esp_pm`'s DFS
  logic and FreeRTOS's tickless-idle behaviour live *inside* precompiled static libraries
  (`libesp_pm.a`, `libfreertos.a`, `libesp_hw_support.a`, etc.) shipped with `framework-
  arduinoespressif32-libs`. A `-D` build flag only affects translation units *your* project compiles; it
  cannot change behaviour already frozen into someone else's `.a` file. Confirmed: the esp32s3 package has
  exactly one shared `sdkconfig` across every memory-type variant (`qio_opi`/`dio_opi`/`opi_opi`/etc. —
  those differ only in flash/PSRAM bus mode, not power management), so there's no alternate prebuilt variant
  to switch to either.
- **Moving from Arduino IDE to VS Code did not remove this ceiling** - both use the same prebuilt-library
  build strategy (`framework = arduino`) under PlatformIO. What *would* unlock real DFS is switching the
  framework itself to compile ESP-IDF from source with a custom `sdkconfig` - either `framework = espidf`
  with Arduino pulled in as a component, or PlatformIO's newer arduino-as-idf-component hybrid. That's a
  real, substantially bigger undertaking: CMake-based project restructuring instead of a single `.ino`, a
  much longer first build (compiling the whole IDF tree from source vs. linking prebuilt archives - for
  reference, the OMOTE reference project's prebuilt-lib `.pio/` cache alone is ~2GB and takes minutes to
  rebuild; a from-source IDF build is a bigger jump still), and real risk that some Arduino compatibility
  shim behaves differently once compiled fresh.
- **Even if that migration were done, DFS still might need to stay off for BLE reliability specifically.**
  The existing "unsafe on Rev 5" comment reflects a well-documented class of real bugs in the Arduino-ESP32
  ecosystem: BLE/Wi-Fi coexistence with automatic light-sleep and dynamic frequency scaling has a real
  history of dropped connections and missed HID events. That's very likely *why* Arduino-ESP32 ships PM
  disabled by default for boards like this, independent of which framework produced the binary. Any future
  attempt at real DFS would need genuine on-hardware validation of BLE HID reliability, not just a
  successful build.
- **Not pursued this session** - documented as a known, understood limitation rather than attempted. If
  revisited, treat it as its own project (framework migration) separate from anything BLE/battery-specific,
  and expect to re-validate the entire BLE-connected-idle path afterward.
...
- **DFS/CPU frequency below 80MHz during BLE idle.** See the new section above — this is now the most
  thoroughly documented "why not" in this handoff.
...
- **CPU stays at a fixed 80MHz during BLE-connected idle, never lower** — see the new DFS section above.
  The radio-side savings from 2.95 are real and independent of this; the CPU-side ceiling is a separate,
  much bigger undertaking if ever pursued.
...
4. **DFS/CPU frequency below 80MHz during BLE idle is a known, understood, unpursued limitation** — see the
   dedicated section above. Only relevant if the user wants to consider an ESP-IDF-from-source framework
   migration specifically to chase it; not a small change.
5. **No end-to-end restore test of every backup category** since the 2.92 fix — a full backup restores
   (confirmed), but backup → wipe → restore → sync → verify icons/themes/devices as one full cycle has not
   been done.
6. Everything else carried over unverified from the prior handoff (WebConfig's stale `FIRMWARE_CHANGELOG`,
