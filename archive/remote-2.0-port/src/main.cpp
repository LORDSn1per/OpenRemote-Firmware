...
int main(int argc, char *argv[]) {
  ...
  // OpenRemote_2.0 foundation: only Settings and the fake Activities page are
  // shown as menu tabs for now. The device/scene GUIs below stay registered
  // (their command bindings are still referenced by sceneRegistry's default
  // key map) but are intentionally not added to the tab list - see main_gui_list
  // override below.
  //register_gui_sceneSelection();
  //register_gui_irReceiver();
  register_gui_settings();
  register_gui_activities();
  ...
  main_gui_list = {tabName_settings, tabName_activities};
  #endif
  ...
  setLabelActiveScene();
  // restore persisted Debug menu state (FPS / touch overlays) now that the
  // overlay objects exist (created inside init_gui() -> init_debugOverlay())
  setShowFPS(get_debugFPSEnabled());
  setShowTouchDebug(get_debugTouchEnabled());
...
void loop(unsigned long *pIMUTaskTimer, unsigned long *pUpdateStatusTimer) {
  ...
  if(millis() - *pUpdateStatusTimer >= 1000) {
    *pUpdateStatusTimer = millis();
    ...
    // recompute and display the FPS counter, if enabled
    update_debugOverlay_fps();
  }
