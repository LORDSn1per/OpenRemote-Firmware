...
def inspect_firmware_file(path):
    ...
    if chip_id!=9:
        raise RuntimeError("The selected firmware is not built for an ESP32-S3.")
    app_size=len(data)-app_offset
    if app_size>0x330000:
        raise RuntimeError("The selected firmware is too large for OpenRemote's application partition.")
    marker=re.search(rb"OPENREMOTE_FIRMWARE_VERSION=([0-9]+(?:\.[0-9]+)+)",data)
...
def selected_factory_image():
    ...
    if info["kind"]=="factory": return info,Path(info["path"])
...
def detect_factory_board(port):
    ...
    try:
        with USB_LOCK:
            run_esptool(["--chip","esp32s3","--port",port,"--baud","115200",
        ...
        FACTORY_STATE.update({"running":False,"done":True,"progress":100,
                              "status":"ESP32-S3 detected and ready to install OpenRemote."})
        return {"ok":True,"port":port,"chip":"ESP32-S3","selection":SETUP_SELECTION}
    except Exception as error:
...
def flash_factory_board(port):
    ...
    if not SETUP_SELECTION.get("boardDetected") or normalize_usb_port(SETUP_SELECTION.get("detectedPort",""))!=port:
        raise RuntimeError("Check this ESP32-S3 board before installing firmware.")
    reset_factory_state("flash","Connecting to the ESP32-S3 bootloader...","setup")
    FACTORY_STATE["progress"]=2
    try:
        manifest=factory_manifest()
        firmware,image=selected_factory_image()
        with USB_LOCK:
            run_esptool([
        ...
        FACTORY_STATE.update({"running":False,"done":True,"progress":100,
            "status":"Firmware "+str(firmware.get("version"))+" installed and verified."})
...
def flash_sensor_test_board(port):
    ...
    if not port: raise RuntimeError("Choose the ESP32 USB serial port first.")
    if not SETUP_SELECTION.get("boardDetected") or normalize_usb_port(SETUP_SELECTION.get("detectedPort",""))!=port:
        raise RuntimeError("Check this ESP32-S3 board before installing Sensor Test.")
